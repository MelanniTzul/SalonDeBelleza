...
@Component({
  ...
  imports: [Lightbox, MenuUsuario, NgOptimizedImage, RouterLink, RouterLinkActive, RouterOutlet],
      ...
      <header class="sticky top-0 z-30 border-b border-line bg-ivory/90 backdrop-blur">
        <div class="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-3.5 lg:px-8">
          ...
          <div class="flex items-center gap-2.5">
            @if (auth.autenticado()) {
              <a [routerLink]="rutaPanel()" class="hidden rounded-full border border-wine px-[17px] py-2 text-[13px] font-semibold text-wine transition hover:bg-blush md:inline-block">Mi panel</a>
              <app-menu-usuario />
            } @else {
        ...
        @if (menuAbierto()) {
          <nav id="menu-movil" aria-label="Menú móvil" class="border-t border-line bg-ivory md:hidden">
            <div class="mx-auto flex max-w-6xl flex-col px-6 py-2">
              ...
              @if (auth.autenticado()) {
                <a [routerLink]="rutaPanel()" (click)="cerrarMenu()" class="py-3.5 text-sm font-semibold text-wine">Mi panel</a>
              } @else {
...
export class PublicLayout {
  ...
  // A dónde lleva "Mi panel" según el rol de la sesión
